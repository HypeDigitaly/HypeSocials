...
FR-70 (`run_id` shape), FR-71 (asset-id encoding, the 40-character trend-slug cap, uniqueness
...
from hypesocials.models import AssetRecord, PlanEntry, TrendItem
...
ASSET_ID = re.compile(r"^(Li|Ig|Tk)_(img|car|reel)_[a-z0-9][a-z0-9-]*_(analyzed|direct)_\d{2,}$")
...
def _trend(key: str, name: str, *, strength: float = 0.9, slideshow: bool = False) -> TrendItem:
    return TrendItem(
        ...
        reference_groups=[["https://cdn/1.jpg", "https://cdn/2.jpg"]],
    )


def _assigned(cfg: Config, trends: list[TrendItem]) -> list[PlanEntry]:
    """Expand a plan and bind trends to it — the two steps that mint a final asset id."""
    plan = build_plan(cfg)
...
def test_fr71_asset_id_encodes_platform_format_slug_variant_and_ordinal() -> None:
    """FR-71: "asset_id encodes: platform (Li, Ig, Tk), format (img, car, reel), a URL-safe slug
    of the source trend name …, variant tag … and a zero-padded per-run ordinal"."""
    cfg = _config(formats={"image": 3, "carousel": 2, "reel": 0}, generation_mode="analyzed")
    ...
    for entry in entries:
        ...
        platform, fmt, slug, variant, ordinal = entry.asset_id.split("_")
        ...
        assert variant == entry.variant
        assert int(ordinal) == entry.order + 1 and len(ordinal) >= 2

    # the PRD's own example is the same shape
    assert ASSET_ID.fullmatch("Li_car_dance-challenge_analyzed_02")


def test_fr71_ids_are_unique_within_a_run_even_when_everything_else_matches() -> None:
    """FR-71: "two creatives sharing platform + format + trend + variant receive distinct
    ordinals to prevent silent folder overwrites"."""
    cfg = _config(formats={"image": 6, "carousel": 0, "reel": 0}, platforms=["linkedin"],
                  languages={"linkedin": "en"}, generation_mode="analyzed")
    ...
    assert {id_.rsplit("_", 1)[0] for id_ in ids} == {"Li_img_dance-challenge_analyzed"}
    assert [id_.rsplit("_", 1)[1] for id_ in ids] == ["01", "02", "03", "04", "05", "06"]


def test_fr71_both_mode_siblings_share_a_pair_id_but_never_an_asset_id() -> None:
    """FR-22/40 §2: "both variants share a `pair_id` … but remain separate asset folders"."""
    cfg = _config(formats={"image": 1, "carousel": 0, "reel": 0}, generation_mode="both")
    entries = _assigned(cfg, [_trend("dance", "Dance Challenge")])

    assert len(entries) == 2
    assert entries[0].pair_id == entries[1].pair_id is not None
    assert entries[0].asset_id != entries[1].asset_id
    assert {e.asset_id.split("_")[3] for e in entries} == {"analyzed", "direct"}


def test_fr71_the_trend_slug_inside_an_id_is_capped_and_windows_safe() -> None:
    """A 90-character trend name with punctuation still has to make a writable folder name."""
...
def test_fr71_an_unassigned_entry_still_carries_a_usable_provisional_id() -> None:
    """The plan is logged and budgeted before Collect runs, so an id exists before a trend does —
...
def test_fr71_a_brief_creative_takes_its_slug_from_the_brief_name() -> None:
    """40 §2: brief-driven creatives (override, no trend) get a "slug derived from the brief
    name" — `assign()` leaves them alone because they consume no trend (FR-144)."""
...
def test_fr71_the_asset_folder_name_is_the_asset_id_verbatim(tmp_path: Path) -> None:
    ...
    folder = AssetFolder(run_dir, AssetRecord(
        asset_id="Li_car_dance-challenge_analyzed_02", source="dance",
        source_name="Dance Challenge", platform="linkedin", creative_format="carousel"))

    assert folder.path == run_dir / "Li_car_dance-challenge_analyzed_02"
...
def test_fr71_distinct_ordinals_make_distinct_folders_and_never_overwrite(
    ...
    """The reason the ordinal exists: same platform + format + trend + variant, two folders."""
    run_dir = create_run_folder(tmp_path, new_run_id())
    ids = ["Li_img_dance-challenge_analyzed_01", "Li_img_dance-challenge_analyzed_02"]
