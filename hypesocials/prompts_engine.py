...
_ALLOWLIST: dict[str, frozenset[str]] = {
    ...
    # v2.7.0 (D63, FR-343/344) — the copy role's THIRD contract: translate a bound deck's admitted
    # panels into the platform's configured language, never shortening them. Same standing context,
    # same siblings, same budgets line as the two copy roles above (the budgets line takes its
    # VERBATIM branch — `copywrite._call_translate` passes `carousel_copy_mode="verbatim"` — so the
    # prompt never states a per-slide ceiling; that absence is the no-shortening guarantee made
    # structural). ONE slot swapped: `translate_panels` carries the admitted panels numbered by
    # source position WITHOUT a budget, and it is allowlisted HERE AND NOWHERE ELSE on exactly the
    # `compress_panels` terms — third-party source text handed to a model that is asked to write new
    # bytes from it. Neither copy sibling may name it: the selection role would be handed text to
    # retype, the compress role a licence to rewrite in another language (its own rule 2 forbids
    # translating). A role that drifts into naming it does not resolve (FR-260/261).
    "copy_translate_system.md": frozenset({
...
""",
    ...
    # v2.7.0 (D63, FR-343/344) — the carousel TRANSLATE call, the copy role's THIRD contract. BYTE-IDENTICAL to prompts/copy_translate_system.md, like every live entry in this table;
    # spliced by plans/tools/splice_builtin_twin.py — change the file, re-run the tool in the same commit.
    "copy_translate_system.md": """ROLE
