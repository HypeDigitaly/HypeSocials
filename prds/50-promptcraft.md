...
- The **meta-style registry** (`prompts/styles.yaml`) supplies the visual substance — colors, typography, layout, motion profile — already written once and reused (D41). Eight textual style definitions, deterministically rotated per creative.
...
**FR-181** *(amended v2.0.0, v2.1.0)*: Templates are files, hot-loaded per run, in a two-level layout. The nine prompt roles split by what owns them:

- **Five global role templates, flat in `prompts/`** — `copywriter_system.md` (verbatim copy reference selection), `vision_check_question.md` (visual quality review), `topic_filter_system.md` (competitor filter screen), `slide_intel_question.md` (per-slide transcription + visual brief, v2.1.0 — new), and the **meta-style registry `styles.yaml`** (8 textual style definitions). These belong to the OpenRouter LLM roles or the visual authority, not to any render-model profile, and exist exactly once.
...
- The existing `vision_check_question.md` template (unchanged from v1.x) carries FR-27's binary quality judgment: pass or suggest a specific re-render change (text shortening, focal element adjustment, etc.). The `analysis` role survives as the vision-check lane only, no style-brief analysis. **Defect categories (v2.1.3, D48):** `text_broken` includes illegible, garbled, or low-contrast on-image text (e.g., dark text on dark background, letters rendered as noise or symbol soup, headlines unreadable even if letterforms are technically well-formed); missing or malformed sanctioned tool marks (FR-315); missing required slide-counter elements (FR-313); misplaced or duplicated text. When a `text_broken` defect is detected, the vision-check retry guidance explicitly names the defect and the expected correct rendering, enabling the re-render prompt to quote it back with correction instruction per Amended FR-105.
