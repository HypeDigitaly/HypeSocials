...
**FR-308 — Visual-brief rendering: content directive subordinate to style DNA.** Each carousel slide's render prompt carries that slide's `visual_brief` (per-slide content directive, returned by the slide-intelligence step per `20-integrations.md` FR-306) as a distinct, labeled section positioned between STYLE_DNA and CONSTRAINTS. The brief describes the visual *content* of that slide in English — layout, graphics, icons, charts, composition — *not* the style/look/treatment, which the style DNA already governs. **Brand safety:** competitor logos, wordmarks, or product names appearing in the brief or detected in the slide's `brand_marks` list are genericized in the render instruction ("a generic chat-app logo shape"), never reproduced or named as a rendering instruction. Platform chrome, watermarks, and usernames are excluded outright from render directives. On-image text (from source panels) stays verbatim in the source language and is never genericized.
