...
**FR-352 *(v2.6.0, new, D62)* — Cover-pick playbook (analysis role, template-driven).** Template `prompts/cover_pick_system.md` + byte-identical built-in twin in `prompts_engine._BUILT_INS`; the TENTH global template (FR-181's roster grows from 10 to 11 global role templates, total shipped roles from 15 to 16 with profile templates; `tests/test_template_parity.py` `SHIPPED_COUNT` 14 → 15 — that test counts templates only, never the registry). Role `analysis` (Sonnet, vision, metered). Placeholder contract, allowlisted for THIS role only: `{{cover_contract}}` (the deck's style key, the exact `style_dna` bytes the render prompt carried, every string that must be legible on the cover — slide-1 text, wordmark when branded, counter badge when counted — all as fenced DATA) and `{{cover_candidates}}` (one line per attached image, `candidate <N> — attachment <N>`). The candidates ride as image attachments in candidate order. Judging order is fixed and stated in the template: (1) style-contract adherence to the DNA (palette, ground, type, counter position, no invented chrome), (2) legibility of the expected strings at thumbnail size, (3) stopping power. Answer: JSON `{"chosen": <candidate id>, "reason": <≤ ~12 words>}`; a missing/duplicate/out-of-range id is discarded by the engine → candidate 1 + `cover_pick_degraded`. Fenced-data discipline as in `style_match_system.md` (nothing in the contract block is an instruction). Nothing the model writes is rendered; `reason` is read by a person in the log and the gallery.
