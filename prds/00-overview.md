...
**Amendment: v2.0.0 (2026-08-12)** — Topic-first pivot: Virlo text-only source, local meta-style registry (8 styles + own reference images), verbatim copy via reference selection, competitor filter, branding injection, motion references removed, A/B mode removed. Core premise inverted: visual authority is the registry (not vision analysis); copy is selection (not generation). See amendment log for full details.
...
- **Pick the strongest ones.** It scores each topic on its posts' views, freshness and engagement, skips topics it already used recently, and hands the strongest ones to the creatives you asked for. Topics with mostly image-heavy posts go to slide-decks; others go to pictures and videos.
- **Quote the originals verbatim.** An AI picks exact words from the topic's winning posts — captions, hooks, text overlays, descriptions — and selects one that fits the style's text budget. What renders is a real quote from what went viral, in its original language.
- **Apply a visual style.** Every creative gets one of eight defined visual styles — photoreal ambient, editorial carousel, meme-caricature panels, and others. Each style carries its own palette, typography rules, layout guidance and two reference images. The style pick is deterministic per creative, so the same topic never looks the same way twice in a single run.
...
- **Make the pictures.** All picture jobs are sent at once, each with the style's reference images attached so the results actually match the defined look. For slide-decks, slide 1 renders first and becomes the template for slides 2–N, so the deck is consistent.
...
- **Optional spell-check.** If enabled, an AI looks at each picture — including a reel's seed frame — and answers: is the text broken or does it show fake platform UI? If yes, one retry with shorter text. Then it ships. For slide-decks, slide 1 is checked before the rest are made.
...
The new approach inverts the premise: **visual authority is a local registry of 8 textual style definitions** (palette, typography, layout rules, with their own 1–2 reference images each, stored in the repo and edited like code), rotated deterministically per creative. **Trend topics supply the copy verbatim** — exact quotes from winning posts in their source language, selected and filtered by an AI, never retyped. This reduces hallucination, removes the 260-line vision-analysis stage, and makes fidelity auditable: every word that appears is from a real post; every image style is named and versioned.
...
- **G4:** Visual fidelity achieved via deterministic local meta-style registry (8 textual styles with their own reference images) rotated per creative; style adherence measured per FR-150/232.
...
- **Deliberately absent (post-pivot):** No Virlo media downloads (text-only source); no A/B mode; no motion-reference video for reels; no programmatic logo compositing (wordmark rendered as text via the image model).
...
flowchart TD
    subgraph Inputs["IN — what a run feeds on"]
        V["Virlo via MCP:<br/>trending topics, post captions,<br/>hooks, text overlays, stats<br/>(text and metadata only)"]
        STY["Meta-style registry<br/>`prompts/styles.yaml`:<br/>8 textual styles +<br/>1–2 reference images each"]
    ...
    V --> SPLIT["Split monitor(s)<br/>into topics:<br/>up to 9 per monitor,<br/>per-topic strength"]
    
    SPLIT --> FILT["Filter topics:<br/>competitor blocklist +<br/>fenced LLM screen →<br/>keep / strip / skip verdict"]
    
    FILT --> SEL["Select topics:<br/>rank by strength,<br/>skip recent history,<br/>assign to creatives"]
    ...
    ASSIGN --> COPY["Verbatim copy:<br/>Luna selects refs from post<br/>captions, hooks, text overlays;<br/>engine resolves bytes;<br/>source language kept"]

    subgraph Create["CREATE — two waves, concurrent submissions"]
        COPY --> IMG["IMAGES → GPT Image 2:<br/>prompt + style reference images"]
        COPY --> CAR1["CAROUSEL slide 1 (anchor)<br/>→ GPT Image 2:<br/>style references + checked FIRST"]
        COPY --> SEED["REEL seed frame<br/>→ GPT Image 2:<br/>hook text baked in"]
        CAR1 --> CAR2["CAROUSEL slides 2–N<br/>→ GPT Image 2:<br/>anchor as primary reference,<br/>chained after slide 1"]
        SEED --> REEL["REEL animation<br/>→ Seedance 2.5:<br/>seed frame reference,<br/>native audio, no motion reference"]
    end

    STY -.->|1–2 style reference images<br/>per job; each file uploaded once<br/>per run via memo| Create
...
5. **Quote:** Luna selects exact words from the topic's top posts (captions, hooks, text overlays) that fit the style's character budget and are free of @handles, URLs, emoji. Engine resolves to bytes verbatim. Source language is kept, no translation.
6. **Generate:** Prompts (style metadata + copy selections + style reference images) submitted concurrently in two dependency waves; style reference images uploaded once per run via Kie's file API (24 h retention). Image and carousel slide 1 generated first as primaries; carousel slides 2–N anchored to slide 1; reel seed frame generated; reel animation fed seed frame URL (no motion-reference video post-pivot).
7. **Check** (optional): Vision check via Sonnet 5 ("is text broken?") on images, carousel anchor, reel seed frame, with ≤1 retry. Slides 2–N skip vision check (anchor already passed). Final reel clips never vision-checked.
...
**D32 — Re-based (v2.0.0):** Local reference images (style images + brief images when present) reach Kie via file-upload API (host `kieai.redpandaai.co`; uploads auto-delete ~24 h — same-run-only URLs). Style reference window rotates per reuse; each job lists style images first (they win in conflicts per FR-94). Memo tracks per-run uploads so each file is uploaded once.
...
**D41 — NEW (v2.0.0):** Meta-style registry replaces vision analysis as the visual authority. Eight textual style definitions (palette, typography, layout, with 1–2 reference images each) are stored in `prompts/styles.yaml` (repo-root resolved; FR-174 seam), validated at pre-flight (FR-295 refusal on unusable), and rotated deterministically per creative on `entry.order` with brand/format affinity filters. No built-in fallback tier — the registry is the only visual contract. Rationale: turns visual fidelity into an auditable version-controlled fact (style X has these colors and these images); removes 262 lines of vision analysis; moves style customization from "write a brief vision prompt" to "edit the YAML and add reference images" (operator control, no model).

**D42 — NEW (v2.0.0):** Copy is selection, not generation. Luna selects exact words from trending posts' source strings (captions, hooks, text overlays, descriptions), filtered by: (a) style's on-image character budget (emoji/@/URL/hashtag-free for rendering), (b) competitor blocklist (never shippped), (c) on-the-fly strip of incidental competitor mentions. The engine resolves selections to bytes verbatim (no retyping, no accent loss, no trimming). Source language is kept, never translated. Verbatim verifier asserts every rendered string is a substring of the chosen source post, minus logged strips. Degrade path (no candidate fits budget): caption-only creative, tagged `NO_ONIMAGE_TEXT`, shipped as delivered. Copy-call failure falls back to top post's caption verbatim, tagged `COPY_DEGRADED`, stays a code-1 loss. Rationale: eliminates hallucination (every word is from a real post); reduces cost (~210 lines removed from copywrite); makes output auditable (trace copy to source URL in metadata); operator accepted legal exposure from verbatim company mentions in captions (legal precedent: news aggregators are allowed).
...
- **Fidelity:** Measured as style adherence to the assigned registry entry (colors, typography, layout, motion profile) and topical accuracy (post selection matches the topic trend), **one assessment per run** — the batch as a whole — rated optionally by operator at gallery review (1 = poor, 2 = acceptable, 3 = strong). Target: ≥80% of rated runs score 2 or higher.
...
- **OQ-17 — CLOSED (2026-08-09):** Kie's `gpt-image-2-image-to-image` route accepts references (`input_urls`, ≤16); text-to-image does not.
...
5. ~~**GPT Image 2 reference honoring**~~ — **CLOSED** (spikes/RESULTS.md §B).
...
| **10-pipeline.md** | Detailed run flow, decision logic, edge cases, failure modes. FR blocks: 1–29, 90–109, 141–149, 200–209, **290–291, 293**. |
| **20-integrations.md** | MCP configs, OpenRouter, Kie.ai, render-provider interface. FR blocks: 30–49, 110–129, 160–169, 240–249, 270–279, **293**. |
| **30-configuration-and-run.md** | Config schema, run.bat, menu, CLI flags. FR blocks: 50–69, 130–140, 170–177, 250–259, 280–289, **292, 299–300**. |
| **40-outputs-and-logging.md** | Folder structure, gallery, run log, events, metadata. FR blocks: 70–89, 150–159, 230–239, **296–298**. |
| **50-promptcraft.md** | Prompt design, templates, playbooks, registry. FR blocks: 180–199, 260–269, **290–291**. |
...
**FR-Range Registry (post-amendment):**
  - **10-pipeline:** FR-1–29, 90–109, 141–149, 200–209, FR-290, FR-291, FR-293 (topic extraction contract).
  - **20-integrations:** FR-30–49, 110–129, 160–169, 240–249, 270–279.
  - **30-configuration:** FR-50–69, 130–140, 170–177, 250–259, 280–289, FR-292, FR-299, FR-300.
  - **40-outputs:** FR-70–89, 150–159, 230–239, FR-296, FR-297, FR-298.
  - **50-promptcraft:** FR-180–199, 260–269, FR-290, FR-291, FR-294.
  - **60-publishing:** FR-210–229.

**Next fresh block: FR-301+** — for future amendments.
