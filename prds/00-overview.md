...
HypeSocials target: **~9,500 lines of Python (hard ceiling 10,000 — G2 v1.6.4)**, ~8 creatives in **≈3 minutes (images/carousels) or ≈8–10 minutes (with reels)** — fully concurrent, zero mandatory gates, and maximum visual fidelity to Virlo trends.
...
- **G2** *(restated v1.6.4, operator decision 2026-08-09)*: **Target ~9,500 lines of production Python; hard ceiling 10,000.** Raised from 6,000/6,500 after Wave 2 landed at 7,868 measured lines with Waves 3–6 unbuilt (~1,500–2,000 estimated remaining) — every W2 module overshot its area estimate at the same approved contract/docstring density as Wave 1; the operator chose accepting the real size over docstring stripping or feature cuts. (History: v1.6.3 raised 3,000/4,500 → 6,000/6,500 at the W1 barrier; the original budget was set before the review-hardened contract/docstring density was chosen.) Tracked with `wc -l` at every wave barrier; the escalation clause below is unchanged: if the build trends past 10,000, cuts come from REVIEW-v1.6-recommendations.md by operator decision, never silently. Includes the Virlo MCP wrapper (vs. the old ~37,700 — no Pillow fallback, no disabled harness). The v1.6.1 simplification round (niches-as-configs, no crop/pad, collapsed metadata, one spend table, no text-only machinery, trimmed Phase-2 flags) cut ~590 estimated lines from the v1.6 sizing of 4,000–5,200; if the build trends past 4,500, further cuts come from the remaining candidates in REVIEW-v1.6-recommendations.md (the operator chose to keep `both` mode, the provider seam, and both preview modes).
