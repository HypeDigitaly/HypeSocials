...
- **v2.1.1 (2026-08-13)** — SESSION G Wave 4 docs batch: FR-13 adds caption-substance sentence (≥25 non-hashtag chars, else topic+niche fallback); FR-99 rules on bound panel-mapped deck fallback (panels render verbatim, lose angle selection); FR-107 clarifies estimate prices the ceiling; FR-90 scoped to pre-flight formats guard; FR-73 widens `no_fresh_post_available` gloss (two emitters), adds `bound_post_missing` vocabulary decision and `ref_source` field; FR-305 maps four-filters to three-counter funnel; FR-295 tombstones reference-image existence clause (post-D46); FR-191 fixes cite rot (FR-144/145/95/24/18); `styles.refs_per_job` already removed from config. No flow or decision changes.
