...
Exactly as decided in D8, refined by D19/D20/D26/D27/D29. When the engine starts without `--yes`, it first offers an action choice with four single-key options: [1] Start a new run (default, continues below), [2] Publish a finished run (D29), and [4] List my Virlo monitor IDs (a $0 setup helper replacing the need to know `--list-monitors` — FR-285). For the default path [1], it then shows a short, fixed sequence of seven numbered prompts:
...
**Non-default actions bypass the sequence.** Selecting [2] "Publish a finished run" from the entry-point action choice bypasses the seven-step menu entirely and goes straight to run selection (`run_id` or `latest`); in MVP it is a Phase 2 placeholder that reports publishing isn't implemented yet and exits (D29, `60-publishing-postiz.md`). Selecting [4] "List my Virlo monitor IDs" opens a Virlo session, prints every available monitor's id and name, and exits at $0 — a setup helper (FR-285, FR-251).
...
**FR-56** *(amended v1.8.0)* The engine SHALL present an initial action choice ([1] Start a new run / [2] Publish a finished run / [4] list my Virlo monitor IDs — a setup helper, $0 — FR-285), then for the default path [1], a short sequence of exactly seven operator prompts: (1) config picker, (2) source picker, (3) formats & counts, (4) spend cap, (5) mode & Notion, (6) briefs (optional), (7) confirm — following the numbering stated in this section. There is no separate niche-pack listing (FR-173). Each step SHALL carry a purpose line explaining what the step does and what a good value looks like (FR-284).
...
| `--preview-sources` | Runs Collect **and Select's filters**, then prints every trend with the verdict a paid run would reach — `eligible`, `excluded (history, last used <date>, no unused post left)`, or `unusable (<reason>)` — and exits. Zero billable calls, zero yt-dlp downloads, zero Kie uploads (D19, FR-139). |
