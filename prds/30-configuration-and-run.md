...
- Visual style comes from a **meta-style registry** — 8 text descriptions of visual styles (palette, typography, layout), each with 1–2 local reference images, rotated deterministically across the creatives in your run. The registry is versioned, hashed, and editable in `prompts/styles.yaml` — tune the styles or add more without touching code.
...
5. **Styles and branding are already set.** The style registry ships in `prompts/styles.yaml` with 8 ready-to-use styles (photoreal, editorial, print, meme, anime, UGC, platform showcase, and HypeLead brand card), each with reference images and rotation rules built in. The branding config ships with two brand systems (HypeDigitaly and HypeLead) fully configured. You can edit both YAML files to customize styles or override brand colors, or leave them as-is and run.
...
- Trend and post-recency window (`trend_history_days`, default **7**; `0` disables it) — exclusion applies at both trend and individual-post granularity (FR-153, FR-7) (D12).
...
**Branding configuration (D43)**
- **Top-level `branding:` block** replaces the retired `niche.brand` keys. It carries: `brand` (selector: `hypedigitaly` or `hypelead` — never mixed), `brand_ratio` (0..1 — the fraction of creatives that receive a brand wordmark), `mode` (`background_tint`, `overlay`, or `both`), `placement` (`bottom-center`, etc.), `competitors` (deterministic blocklist, fail-closed), and `profiles` (two brand-system bundles, see below). Notion brand facts are a future override, not a dependency — the config keys are the MVP source.
...
- Kie.ai job timing: `image_job_timeout_s` (default **180**), `video_job_timeout_s` (default **600**), `poll_interval_s` (default **3**) — the gap between `recordInfo` polls for every in-flight job (20-integrations.md FR-43). *Rationale for 600 (v1.6.7):* both live Seedance renders measured **302 s and 378 s** (spikes/RESULTS.md §C) — the earlier 300 s default would have timed out real, paid, never-resubmitted jobs seconds before completion. A genuinely stuck reel at 600 s still leaves room inside `run_deadline_min` 25, and FR-108's grace-poll/abandon path bounds the run regardless.
...
**FR-170** *(amended v2.0.0)* `reel_audio`, `reel_duration_s`, `reel_resolution`, `nsfw_checker`, `reasoning_effort` (copy role), and `sources.virlo_topics_per_monitor` SHALL each be exposed as documented, independently overridable config keys, defaulting per D22 and Section 2 (v2.0.0 removals: `reel_video_reference`, `reel_reference_max_s`, `inspiration_mix` — v1.6.1 removal: `image_quality_tier`).
...
**FR-295** *(v2.0.0, new)* Pre-flight validation SHALL load and parse the meta-style registry from `prompts/styles.yaml` (or the override-first path per FR-174), validate that it contains ≥1 style usable under the active `branding.brand` (error if zero; warning if <3), check that every requested creative format has ≥1 affine style under the active brand (error if not), and verify that every referenced image file exists and passes magic-byte checks for its declared type (warning only if missing — style degrades to text-only). A missing, unparseable, or unusable registry SHALL exit with code 2 naming the file and reason, before any billable call (the same posture as a malformed profile in FR-281). The registry is exempt from FR-183's fallback-to-built-in rule — no built-in registry exists, so a broken registry is a blocking error, not a degradation.
...
**FR-259** *(amended v2.0.0: `reel_reference_max_s` removed — motion references withdrawn, D23/D44)* `poll_interval_s` (3), `mcp_startup_timeout_s` (20), `mcp_call_timeout_s` (30), `http_max_attempts` (3), `virlo_session_pool` (3 — the bounded pool of Virlo wrapper sessions for Collect-stage concurrency, 20-integrations.md FR-246), and `text_budgets` (`image_headline` 42, `image_subline` 60, `reel_seed_headline` 32, `retry_reduction_pct` 40) SHALL each be exposed as documented config keys at those defaults; `video_job_timeout_s` SHALL default to **600** (v1.6.7 — both live Seedance renders measured 302 s and 378 s, spikes/RESULTS.md §C; the previous 300 s default would have paid for both and discarded them seconds before arrival, and a timed-out job is never resubmitted) and `run_deadline_min` to **25**. `http_max_attempts` SHALL govern every bounded-retry path in the engine. (`image_quality_tier` removed v1.6.1 — OQ-7 closed with no tiers exposed.)
...
styles:
  registry_file: prompts/styles.yaml  # local meta-style registry: 8 textual styles + their reference images
branding:
  ...
  competitors: []                   # deterministic blocklist (fail-closed)
...
models:
  ...
  video_job_timeout_s: 1800          # raised 300→600→1800 (v1.6.7→v1.6.9): measured Seedance renders ran 302–378 s; W6 acceptance reel hit 600 s ceiling; default raised to 30 min
...
**FR-56** *(amended v1.8.0, v2.0.0)* The engine SHALL present an initial action choice ([1] Start a new run / [2] Publish a finished run / [4] list my Virlo monitor IDs — a setup helper, $0 — FR-285), then for the default path [1], a short sequence of exactly **five** operator prompts (v2.0.0: the source picker and mode prompt are withdrawn; Virlo is the only source and generation has no mode selector): (1) config picker (showing `lang · monitors · formats · brand · styles` readiness), (2) formats & counts, (3) spend cap, (4) briefs (optional), (5) confirm. There is no separate niche-pack listing (FR-173). Each step SHALL carry a purpose line explaining what the step does and what a good value looks like (FR-284).
...
| `--history-days N` | Overrides `trend_history_days` (the recency exclusion window in days). `0` is valid and turns the window off. A value outside the documented bounds is **refused in one line at the flag boundary, before any config is loaded — never clamped** (FR-285): a silently-corrected typo would hide exactly the kind of operator mistake this refusal exists to surface. |
...
**FR-284** The interactive menu's seven operator prompts (config, source, formats/counts, spend cap, mode/Notion, briefs, confirm) SHALL each carry a short purpose line explaining what the step does and what a good value looks like, placed before the prompt. The source picker and config picker rows SHALL display readiness badges (e.g., "NOT RUNNABLE") for entries that cannot serve a run without additional config. The menu header SHALL advertise the `?` key as a per-step help trigger.
...
| Every analysis call in a run fails | The run **ships direct-mode creatives per FR-12** — all creatives degrade to analyzed→direct fallback and are marked `analysis_missing`. The summary line names the count of degraded creatives (e.g., "all 6 creatives shipped in direct mode due to analysis failure"). Exit code **1** (partial success). |
...
**NFR-16** The interactive menu SHALL present an initial pre-wizard action choice (one key), followed by exactly seven operator inputs — config/niche pick, source pick, formats/counts, spend cap, mode/Notion, briefs, confirm — to reach a started run (eight total prompts); every input is skippable via Enter except the final confirm, and the briefs step in particular defaults to none and is skipped with a bare Enter.
