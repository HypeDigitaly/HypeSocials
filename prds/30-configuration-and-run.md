...
**FR-56** *(simplified v1.6.1)* After an initial action-choice step (one key: Generate / Preview Sources / Preview Analysis), the interactive menu SHALL present a config picker listing all YAML files in `configs/` with their line-1 descriptions (a config with a `niche:` block shows that descriptor line instead) as the first of seven operator inputs. There is no separate niche-pack listing (FR-173).
