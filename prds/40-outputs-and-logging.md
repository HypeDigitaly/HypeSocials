...
- Each MCP server call: name, operation, duration, and a **result summary that carries the row/item count** — e.g., "Virlo MCP: trends → 27 trends found, top 5 confidence ≥ 0.82". The count is required, not decorative: a call that returned nothing and a call that returned a hundred rows must not log identically. *(Gap on record 2026-08-11: the shipped line reads `virlo MCP: get_top_videos -> ok (1971ms)` and omits the count.)*
...
**FR-155 — Virlo funnel report (D39, added 2026-08-11).** A **run-wide rollup** emitted once per run from `virlo.fetch()` showing input volume, qualification, selection, and render attachment — printed **unconditionally** (not gated on `assignment.dropped or interactive`) in three places per §10 rule 2: after Select and before Analyze; in both preview modes; and as one row under FR-84's spend-summary headline. The report lives in `events.jsonl` as a machine-readable `collect_funnel` event with nested `input` / `sets` / `choice` / `images` / `caps` objects. **Input/output vocabulary kept disjoint per NFR-5:** INPUT (Virlo evidence, never rendered): video, slideshow, panel, frame, post, set; OUTPUT (generated): image, carousel, reel, creative, job. Printed lines ≤78 chars per FR-286, trend names never appear (unbounded token). Use ASCII `->`, never `→`. **Worked vocabulary example** — INCORRECT: *"3 slideshows -> 2 carousels"*. An arrow from an input word to an output word asserts a conversion that never happens; a Virlo slideshow is evidence, and it is never rendered. CORRECT: *"the chosen set for 3 of 3 trends came from a slideshow, which is why 2 carousels were planned"* — a slideshow gives the trend **carousel affinity** (FR-90), which is what steers the format decision. **NOT in the pre-flight cost estimate** — Collect happens after Confirm (runner.py:313 after :307), so no Virlo number exists when the estimate prints.

**Event-shape amendments for funnel accountability (FR-155 support, 2026-08-11):**
- **`virlo_payload` event:** Add three fields to the `data` object: (1) `videos_raw` (integer: count of video items returned by endpoint, before dedupe), (2) `slideshows_raw` (integer: count of slideshow items, before dedupe), (3) `total_available` (integer: server-reported total availability). Change the message field from "Fetched X videos, Y slideshows" (ambiguous: pre- or post-dedupe?) to explicitly post-dedupe: "After dedup: X videos, Y slideshows" — matching the counts that enter `refs/<trend_key>/` assignment.
- **`reference_choice` event:** Add (1) `fresh_count` and `stale_count` (how many candidates were excluded by recency window, if any), and (2) `rejection_reasons` (object mapping each rejection reason string to count, e.g. `{ "no_qualifying_motion_reference": 1, "exceeds_reference_cap": 2 }`).
- **`kie_job_submitted` event:** Add (1) `reference_count` (integer: how many reference images/videos attached to this job), (2) `reference_sources` (array of strings describing each source, one per reference image or motion video, e.g. `["Virlo post abc123d, panel 2", "Virlo video def456g", "local Inspiration image.png"]`).
