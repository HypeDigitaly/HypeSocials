...
Each run creates a timestamped folder on disk (`output/YYYYMMDD_HHMMSS_random/`) containing the finished images, carousel slides, videos, captions, and an offline HTML gallery. A single `gallery.html` file shows all creatives side by side with the style assigned to each, the brand applied, and the exact source post (with author, view count, post ID), platform badges, costs—reviewable in ~30 seconds. The folder also includes `run.log` (human-readable step-by-step narrative with stage headers and data tables) and `events.jsonl` (machine-parseable structured data for dashboards). A global `logs/trend_history.json` remembers which topics and posts were used recently to avoid repetition. Failed creatives still appear in the folder with a `SKIP_REASON.txt` file explaining why. Everything is self-contained: no external CDN, works offline, and ready to publish or archive. When using subscription backends (Codex), the same outputs are produced; estimate and spend lines show $0.00 API cost. Claude Code autopilot can additionally read every finished deck and write a post-run review.
...
**FR-72 — Publishable asset enumeration (amended v2.1.0, v2.6.0 D62):** Each successful asset folder contains: the final image file (JPG/PNG, zero-padded if carousel: `slide_01.jpg`, `slide_02.jpg`, etc.), a video file for reels (MP4) **plus, when the seed-frame path ran, the paid seed-frame image as `seed_frame.jpg`** (cover/thumbnail only), a `caption.txt` file holding the caption and hashtags (plain text, ready to paste into a social platform), and a `meta.yaml` file with structured metadata. When `cover_candidates > 1` (carousel only), a `covers/` subfolder stores all candidate anchor renderings as `cover_candidate_1.jpg`, `cover_candidate_2.jpg`, etc., never published, kept for operator inspection. **Canonical publishable set:** `slide_NN.*`, `image.*`, `reel.*`, and `seed_frame.*` (cover-only). **Never published:** the `source/`, `refs/`, and `covers/` subfolders (FR-213 in 60-publishing) and any intermediate analysis artifacts. Only the enumerated asset files move to 60-publishing pipelines.

**FR-73** *(amended v2.0.0, v2.1.0, v2.1.2, v2.3.0, v2.4.0, v2.7.0, D63)*: The `meta.yaml` file is the canonical schema for all asset metadata. It records (grouped logically):

**Identity & sourcing (v2.1.0 amendments: added source_panel_count, panel_map, source_post, ref_source; v2.1.2 amendments: panel_map rows gain creator_stripped; v2.3.0 amendments: top-level copy_mode, panel_map rows gain compressed; v2.4.0 amendments: four style-match fields; v2.6.0 D62 amendments: copy_mode gains auto value, cover_pick field added; v2.7.0 D63 amendments: copy_language, source_language, panel_map rows gain translated):**
- **`copy_mode`** (v2.3.0, new; amended v2.6.0, D62) — `verbatim`, `compress`, or `auto`, tracking which copy contract this asset used (D54, FR-331/FR-353). Gallery and run counters now read compression from panel-map rows (`compressed: true`) rather than mode equality, so mixed auto decks show compress chip only on rows actually compressed.
- **`copy_language`** (v2.7.0, D63, new) — `source` or `target`, tracking which language boundary was applied (FR-345). When `target`, indicates bound carousel text was translated. When `source`, posts kept their original language.
- **`source_language`** (v2.7.0, D63, new) — ISO 639-1 code (two letters, e.g. `de`, `fr`, `""` when unknown) of the source-language texts from the language ladder (FR-343 rung), recorded on every bound carousel where determined, both `source` and `target` modes.
...
- **`cover_pick`** (v2.6.0 D62, new, carousel-only) — JSON object (null for non-carousel or single candidate) containing `candidates` (array of candidate ids ordered 1–N), `chosen` (integer id of vision-picked winner), `reason` (short prose justification from cover-pick analysis), `degraded` (boolean, true when pick fell back to candidate-1 on LLM error/invalid response). Written when `config.cover_candidates > 1` at render submit time.
...
- panel_map (array [optional]: per carousel slide, `{slide, source_position, source_text_original, source_text, drop_reason, creator_stripped, chrome_counter_stripped, ref_label, **compressed, translated**, visual_brief, source_image}` (relative path); empty for override-brief carousels. `source_text_original` is the pre-gate source text; `drop_reason` is one of: empty / contains_handle_or_url / over_budget / creator_stripped (v2.1.2; **v2.1.3 refinement per FR-319: `contains_handle_or_url` fires only for social marks; technical-URL panels are no longer dropped**); `creator_stripped` is boolean, true when the source panel line matched creator identity/chrome per FR-312 and was removed; `chrome_counter_stripped` is boolean, true when a full-line page-counter chrome (e.g. `01 / 06`, `// 02`) was dropped from the admitted panel bytes; **`compressed` is boolean (v2.3.0, D54), true when source_text was LLM-compressed; `ref_label` is empty string for compressed rows**; **`translated` is boolean (v2.7.0, D63), true when source_text is the model's translation to the platform language** (rows `translated: false` either did not translate or shipped source language); `source_text` is the final rendered text (or "" if dropped))
...
**Provenance & degradations (collapsed to one list, v1.6.1; v2.1.0 amendments: degradation vocabulary expanded; cite fixes applied; v2.6.0 D62 amendment: cover_pick_degraded added; v2.7.0 D63 amendments: copy_not_translated, translate_length_drift added):**
- **degradations** — a single list of tags, empty on a clean asset. Defined vocabulary (each tag's behavior owned by the FR that emits it, v2.1.0 list, D62 amended, D63 amended): `copy_not_verbatim` (FR-303), `competitor_stripped` (FR-202), `copy_degraded` (FR-99: LLM copy selection failed; bound panel-mapped carousels render panels verbatim and ship bound post's caption with loss of angle selection; images/reels render minimal sourced text), `copy_not_translated` (FR-343: translation was wanted — mode `target`, carousel bound, source language known and foreign — but the deck shipped source language anyway, either because translate call failed or the entry landed on a degrade path like `_refused`; console reports it loud like `copy_degraded`), `translate_length_drift` (FR-343: an individual translated line measured < 0.5× or > 2.0× its source's length, shipping nonetheless as an audit tag — not a failure), `style_refs_missing` (FR-295: **HISTORICAL** — no emitter since D46 removed the style picture channel; tag remains for gallery rendering of older runs), `no_onimage_text` (FR-100: no candidate fit the style's text budget, caption-only creative), `refs_dropped_moderation` (FR-97: brief-supplied reference images removed on content-policy retry), `text_trimmed` (FR-101: assembled text exceeded budget, trimmed at word boundary), `incomplete` (partial carousel, FR-20/95), `skipped_budget` (FR-106), `abandoned` (FR-108), `seed_frame_render_failed` / `seed_frame_url_unreachable` (FR-24), `vision_transcribed` (FR-306: on-image text supplied by vision analysis), `vision_unavailable` (FR-306: vision analysis failed or timed out, Virlo panels retained), `panels_truncated` (FR-304: carousel deck truncated to platform max), `no_fresh_post_available` (FR-307: **Two emitters:** `plan.assign` skips a creative group when a topic's fresh slideshow posts are exhausted during assignment; `copywrite` refuses a burnt bound post when the bound post ID is absent from the current topic's roster, creating a missing post condition — in either case, entry skipped), `cover_pick_degraded` (FR-352: cover-pick LLM analysis failed or returned invalid id/out-of-range, fell back to candidate-1; gallery shows degraded note beside the picked candidate). The gallery renders every tag as a badge in one loop; new tags need no schema change.
...
**FR-337 — Style-match provenance in outputs (v2.4.0, new; D56).** When matched assignment is enabled, every asset carries four new metadata fields in `meta.yaml` (FR-73 amendments v2.4.0): `style_fit` (high|medium|low), `style_reason` (short prose), `style_origin` (rotation | matched | rotation_fallback), `style_wanted` (wanted_archetype when `fit == low`, else empty). Gallery card labels show assigned style with origin annotation: `style: X · matched/high` (matched-mode pick) or `style: X · rotation` (rotation baseline or fallback). On fallback cards (when matched-mode call fails or entry receives `low` fit), the `style_wanted` field is displayed as a note for operator review ("wanted archetype: listicle card deck"). ASSIGN console receipt gains origin/fit/reason columns per entry. One gap-report block appears after the ASSIGN loop, listing all distinct `wanted_archetype` values found in low-fit entries, so the operator can plan missing styles deliberately. New events.jsonl event type: `style_match` (per-run, captures matcher call outcomes and any degradation; detail-only, never console). The gap block is blind to a pool that is merely STARVED — run `20260820_145809_4a0q` put 6 of 9 decks on one style with `style_wanted` empty on all nine — so since v2.5.2 (D61) FR-355's concentration line below closes that blind spot.

**FR-355 — ASSIGN concentration line (new, v2.5.2, D61).** After the ASSIGN receipts and the FR-337 gap block, the runner counts distinct `PlanEntry.style_key` over the live plan (override briefs with no style excluded) and prints ONE warning line when either (a) one style takes MORE than half the styled creatives, or (b) fewer than 3 distinct styles on a plan of 5+ styled creatives. Pure arithmetic, no LLM, lands in `--preview-analysis`, ≤ 78 chars (FR-286), never a refusal, never a retry. Example (78 chars exactly, hanging under the receipts at their 10-space indent): `          concentration: icon-ledger-carousel 6/9 (>1/2) - pool may be starved`; the distinct-count shape reads `          concentration: only 2 style(s) across 6 - pool may be starved`. Trigger (a) wins when both hold (the strict `count * 2 > total` means an even 3/6 split is a spread, not a bunch). The console says one trigger; the `run.log` warning records both numbers. Silent when neither condition holds (D45: no heading without rows). Logged as a `style_concentration` warning event in events.jsonl with `style_key`, `count`, `total`, `distinct`. It prints in `matched` and `rotation` mode alike, under the ASSIGN receipts at their indent, and is the console half of the gap report (FR-336/FR-337) for the case the gap block cannot see: the matcher answered `high` everywhere and the pool was simply starved (run `20260820_145809_4a0q`, 6 of 9 on `icon-ledger-carousel`, `style_wanted` empty on all nine).

**FR-354 *(v2.6.0, new, D62)* — Per-row compress provenance + cover provenance in outputs.** `meta.yaml.copy_mode` gains the value `auto` (FR-353); a deck's compression is read from its ROWS, never from mode equality — the gallery's "compressed from N chars" label and the per-slide chip are driven by any row with `compressed: true` (so a mixed auto deck shows the chip only on the rows that were compressed, and a deck that needed nothing reads exactly as a verbatim deck). `meta.yaml.cover_pick` (shape in FR-351) is written on every anchored carousel rendered with `cover_candidates > 1`; `covers/cover_candidate_<N>.jpg` is a per-asset sub-folder (FR-72 folder contents amended) — artifacts kept for the operator, never published, never uploaded. Gallery: a cover-candidate strip appears under the style lines of each carousel card — every candidate thumbnail, the chosen one marked, the `reason` beside it, a `cover_pick_degraded` note when the pick fell back. Run summary/preview counters that used to test `copy_mode == "compress"` now count decks with any compressed row.

**FR-346 *(v2.7.0, new, D63)* — Translation provenance in outputs.** When carousel decks are translated (FR-343, mode `target` + carousel bound + source language known and different from platform language), `meta.yaml` records `copy_language: target` and `source_language: <ISO 639-1 code>`; both `source` and `target` modes record `source_language` on every bound deck where determined. Per-row `panel_map[*].translated: bool` flags which rows shipped translated text (true) vs source language (false). Gallery: a card-level **line appears below the style lines** when `copy_language: target`: `Copy: translated from de to the platform's language — …` naming the source language (the gallery reads `meta.yaml` alone, which records `source_language` and no target field — the platform's configured language is config state, so the line names it by role, not by code). Per-row **chip appears beside the compress chip** (when translation happened) showing `translated from de` in the same visual hierarchy. Console: the COPY-stage closing line counts translated decks separately when carousels are present. **Provenance row:** `translated P1 @handle views <id> <src>-><tgt> -> panel_map` shape, showing source topic, operator, audience (views), post id, language direction and mapping reference. **Previews:** `_copy_block` header counts `N deck(s) translated` when carousels are shown. `_source_rows` includes a `translated` row kind (wider than `_ROW_LABEL` like `compressed`, already supported). **Failure paths:** when translation is wanted but does not happen (`copy_not_translated` tag), the gallery shows the `copy not translated` badge through the degradation loop and the CONSOLE is loud — a two-line `copy_not_translated: N deck(s) -- <ordinals>` block on the `copy_degraded` pattern, plus a `run.log` warning carrying the full asset ids (never in the exit-1 `llm_starved` set). A row that drifted in length (`translate_length_drift` tag) earns the badge on the card and a `translate_length_drift` warning in `run.log` naming the slide and both lengths; the provenance block does not repeat the numbers.
...
## 12. Claude Code Autopilot (FR-361)

Post-run review by Claude Code AI, writing advisory summary + log.

**Skill & agents:** `.claude/skills/hypesocials-run/SKILL.md` + agents `hs-operator`, `hs-deck-critic` (defined in skill manifest). `autopilot.bat` in repo root runs `claude -p "/hypesocials-run"` to invoke the skill.

**Task registration:** `plans/tools/register_autopilot_task.ps1` (PowerShell) registers a daily Task Scheduler job to run `autopilot.bat` unattended (no operator token or API key required — Anthropic forbids consumer OAuth in third-party tools).

**Output files:**
- **`output/<run>/CLAUDE_REVIEW.md`** — Post-run summary written by Claude after reviewing every deck in the run. Covers style adherence, fidelity to source, quality issues, and actionable observations. Advisory only; never re-renders or publishes.
- **`logs/autopilot/AUTOPILOT_LOG.md`** — Cumulative log of all autopilot runs, showing timestamps, run ids reviewed, summary line per run, and any errors encountered during review.

**Scope:** Claude is a second, independent critic panel (after the engine's gauntlet gate). It reads pixels only; it never modifies meta.yaml, never deletes or republishes assets, never updates trend history.

**Failsafe:** Autopilot job failure does not affect published outputs. If Claude review fails or runs late, the engine's published creatives are unaffected. Review is delivered asynchronously (next day / next run cycle).

- **FR-361**: The engine repository SHALL include `.claude/skills/hypesocials-run/` skill definition with agents `hs-operator` and `hs-deck-critic`, enabling `autopilot.bat` to invoke Claude review. `plans/tools/register_autopilot_task.ps1` SHALL register an optional daily Task Scheduler job. Post-run Claude review writes `output/<run>/CLAUDE_REVIEW.md` summary + `logs/autopilot/AUTOPILOT_LOG.md` event log (advisory only, no re-render, no publish).
