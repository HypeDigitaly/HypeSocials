...
**Amendment: v2.0.0 (2026-08-12)** — Pivot to topic-first architecture, local meta-style registry, verbatim copy via reference selection, competitor filter, branding injection, motion references removed, A/B mode removed. Core premises inverted: visual authority is the registry (not vision analysis); copy is selection (not generation). See amendment log in `00-overview.md`.
...
- **Quote the originals.** Another AI picks the exact words from the winning posts to use on your creative. It selects from the post captions, hooks, text overlays and descriptions, so what you render is a real quote from what went viral, in its own language.
- **Apply a style.** Every creative gets one of eight defined visual styles — photo-real ambient captions, editorial carousels, meme-caricature panels, and others — each with its own color palette, typography rules, layout guidance and reference images to show the model. The style is deterministic per creative so the same topic never looks the same way twice in a run.
...
- **Make the pictures.** All the picture jobs are sent off at the same time, each one with the style's reference images attached so the result actually looks like the style. For slide-decks, slide 1 is made first and then used as the template for the rest, so the deck looks like one deck.
...
**FR-145 — `blend` mode takes a topic, with the assigned style dominant on visuals (amended v2.0.0).** A blend creative is assigned a topic by the normal rules of FR-8 and FR-90, receives a style from the rotation like any other creative, and counts toward reuse and history as usual. Its prompt assembly adds the brief's directives alongside the style, under a **stated precedence: the assigned meta-style wins on everything visual** — layout, palette, typography, treatment, composition — **and the brief wins on message, offer and CTA**, plus product nouns for the on-image text. Both the style's reference images and the brief's attach (style images listed first, FR-290). This is the same precedence shape as FR-109's rule, and for the same reason: a creative that stops looking like its house style has stopped being on-brand output.
...
**FR-5 — Viral-strength ranking (amended v2.0.0).** Ranking consumes a **`strength` value in 0–1 that each source adapter computes for its own items** — that one number is the cross-source contract (a future Google Trends or Hacker News adapter scores its items however suits its data; nothing in Select knows any source's internals). The Virlo adapter's strength is computed per **topic** (not per trend/monitor): it combines `total_views`, `median_views`, `velocity` and `engagement` from that topic's own `SourcePost` subset — each component **min-max normalized to 0–1 within the run's full topic pool** before weighting with the same hardcoded weights: total views (**0.35**), median views (**0.15**), velocity (**0.30**), engagement (**0.20**). The weights are **hardcoded at those defaults** — no config knob, because tuning weights is a false lever compared with the quality of the style registry — and they are stated here so the operator has signed off on them rather than a builder inventing them. The full ranked list with each component is written to the run log so a human can see exactly why a topic won.
...
**FR-7 — History exclusion window (amended v2.0.0).** The window excludes the **individual source posts already used** within the last `trend_history_days` (per the trend-history state described in the output file). A topic is excluded from the plan only when **it has no unused source-post candidates remaining**. `0` disables the window entirely. Exclusions are logged individually with the date the post was last used, so a thin run is always explainable.
...
**FR-90 — Format affinity beats format weight (amended v2.0.0).** Assignment matches topics to formats by the *composition of their source posts*, not by a fixed "strongest topic gets the reel" ordering:

- Topics whose posts are **majority-slideshow** (image panels + `panel_texts` + narrative arc) are preferred for **carousels** — the posts already are carousels.
- Topics whose posts are **majority-video** (hooks, overlay text, descriptions) are preferred for **images and reels**.
- When no affinity match remains for a format, assignment falls back to plain rank order.

Every affinity decision — which topic went to which creative and whether it was an affinity match or a rank fallback — is logged with its reason, so an odd pairing is always explainable.

**FR-91 — Style rotation, reference-window rotation (amended v2.0.0).** Style assignment is deterministic per FR-291. Where a style has multiple reference images (e.g., hypelead-brand-card's 5 images), the per-job window rotates: creative *k* on a topic attaches `style.reference_images[k % len(reference_images)]`, ensuring visual divergence across siblings while respecting the "one coherent style set per job" rule.
...
**FR-99 — One copy call per (topic × language); copy via reference selection (amended v2.0.0).** Copy is selected, not written. The engine issues **one GPT 5.6 Luna call per distinct (topic × language) pair**, and that single call selects copy for **all sibling creatives** on that topic in that language at once. The prompt names the offerable source strings the engine has pre-numbered from that topic's `SourcePost` list (§1.7) and requires the LLM to return references by number (`headline_ref: P2.hook.1`, `caption_ref: P1.caption`, etc.), never free-text copy. The engine then resolves those references to the actual source bytes — verbatim, always the same text, never retyped.
...
**FR-13 — Copy outputs per format (amended v2.0.0).** Every creative gets a caption and a hashtag set. Beyond that:
- **Image** — one on-image text block (headline plus optional subline), sourced from the selected `SourcePost`'s caption or hooks.
- **Carousel** — per-slide text for every slide (each sourced from the `SourcePost` selection) plus any narrative arc the posts contained. **Per-slide word counts are sourced**: if the selected post's `panel_texts` put four words on panel 1, that fourth post's actual words go there; if the selected post has no panel text for that slot, the on-image text degrades (tagged `NO_ONIMAGE_TEXT`).
...
**FR-100 / FR-101 — Verbatim copy via reference selection (new, v2.0.0).** The source posts are the asset. The engine numbers every offerable candidate (captions without @handles/URLs/emoji, hooks under the style's character budget, panel texts from the selected post), the LLM selects by reference, and the engine resolves bytes — **no retyping, no translation, no trimming outside of word boundaries**. The verifier (post-pivot polarity flip of A20) asserts every rendered on-image string IS a byte-substring of the quoted SourcePost and NEVER contains a stripped competitor brand. No budget trimming at copy time; a string that does not fit was never offered. When no candidate fits the style's budget for a slot, that slot degrades to `NO_ONIMAGE_TEXT` (caption-only creative) and is tagged accordingly.
...
**FR-17 — Prompt assembly (amended v2.0.0).** Every creative's image prompt is assembled deterministically from, in order: the assigned style's `render_prompt` (≤120 words, from the meta-style registry); its `layout_zones` (which content occupies which frame region); the copy's exact on-image text with an instruction to render it verbatim; and the mandatory clauses of FR-94. Assembly fills the model-specific scaffolds from the editable `prompts/` folder — the section order, text-locking phrasing, and per-model conventions are owned by `50-promptcraft.md`. Prompts stay compact because long prompts dilute reference-image influence. The assembled prompt is logged in full for every job (to `events.jsonl`, per `40-…`'s logging split).

**FR-94 — Mandatory prompt clauses (amended v2.0.0).** Every render prompt carries these clauses:

1. **Exclusion clause (mandatory).** Never reproduce platform UI, watermarks, usernames, follower counts, engagement counters, progress bars, or any literal strings present in the reference images. The references inform **layout, palette, typography and treatment only** — never content, never chrome. The style's `exclusions` field (literal wordmark/label strings from the reference files themselves, per §1.3) is appended to this clause.
...
**FR-18 — Style reference images (amended v2.0.0).** The assigned style's reference images (local images from the meta-style registry, per FR-290/291) are attached as **image inputs** to every GPT Image 2 job. This is the visual-fidelity backbone of the product (D2). Up to 3 style references per job are followed by up to 3 brief-supplied references (if the creative carries an override/blend brief with its own reference images); brief references rank below style references (FR-145 precedence). Which images and how many is governed by `config.styles.refs_per_job` (default 3 total).

**An upload failure degrades, it never blocks.** If one reference fails to upload to Kie, the job proceeds with whichever references did upload and the drop is logged by filename with its reason. If every reference in the set fails, the job proceeds with the rendered style guidance alone (no reference URLs attached) — still reference-qualified because the style's textual guidance is intact. A curated reference image is an input, not a prerequisite.
...
**FR-95 — Anchor chaining and the slide-count ceiling (amended v2.0.0).** Per D17, `carousel_anchor` (default `true`) changes the submission shape:
...
- **Slides 2–N are then submitted concurrently** with the finished slide 1 attached as the **PRIMARY reference image**, ahead of the style references, plus a **fixed template-lock instruction**: reproduce this exact template, palette, typography, margins and text placement; change only the text and the focal element.
- **If slide 1 fails**, the carousel falls back to independent generation of all slides from the style references alone, and the fallback is logged. If slide 1 is flagged and its single re-render also comes back flagged, it **ships as the anchor anyway** — one retry is the cap everywhere, and a flagged anchor still beats an unanchored deck.
...
Slide count comes from config per platform (`platforms.<name>.carousel_slides`, canonical in `30-configuration-and-run.md` FR-257). **The config value is the ceiling and the estimate basis.** Topics whose source posts include slideshow panels may **reduce** the slide count — mimicking the original's pacing — but may never raise it above the configured ceiling, so the pre-flight estimate can never be exceeded by a surprisingly long source deck.
...
- **`seed_frame` (default)** — GPT Image 2 first renders a still hook frame **with the hook text burned into it**, using the assigned style, the style's reference set, and the same FR-94 clauses as any image. Because that seed frame is generated through Kie, the provider returns it as a **Kie-hosted public result URL**; that URL is passed **directly** into Seedance's reference-image list, and the prompt points at it using the model's `@Image1` prompt-reference syntax. **There is no upload step and no local file handling in this chain** — the engine never re-uploads the frame it just paid for. The animate instruction states explicitly that **the on-frame text stays static, unmoved and legible** while the surrounding scene moves. Image models render text far better than video models do; this buys legible reel text for the price of one extra image render, which the estimate already counts.
...
**FR-107 — What the estimate must include (amended v2.0.0).** The estimate enumerates every **conditional contributor**, not just the obvious renders, because an estimate that omits half the spend is worse than no estimate:
...
| Style reference image fails to upload to Kie (or download from style registry) | The job proceeds with its remaining references and the dropped file is logged by name (FR-200). If every reference in the set fails, the job proceeds with the rendered style guidance alone (no reference URLs attached). A style is still usable text-only. |
...
| A style's `reference_images` file missing or unreadable at pre-flight | Warning only; that style degrades to text-only for the run, tagged `style_refs_missing` (FR-290). The registry itself missing or unparseable is the exit-2 refusal row above (FR-295). |
...
**D2 — Style-registry reference images always attached (amended v2.0.0).** Mimicry quality is the product. A meta-style is a *description* of what won (derived from curated Inspiration and brand assets); the style's reference images are the *evidence*. Attaching those reference images as input to every job conditions the render on ground truth rather than prose alone. Sibling creatives on one topic each get a different reference-image window from the same style (A17 mechanism re-scoped), ensuring visual divergence while respecting the "one coherent style set" rule.
...
**D23 — withdrawn (v2.0.0, operator decision).** Viral-video motion references via yt-dlp are removed per operator decision 2026-08-12. Reels render from seed frame + style reference images only. See the amendment log at `00-overview.md` and FR-142 withdrawal.
