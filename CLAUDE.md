...
**Last updated:** 2026-08-20 (v2.5.0/D59, SESSION J: contracts & render correctness — FR-338 (`{{counter_rule}}` render slot, five-arm truth table), FR-339 (gated-zone DNA rule: chip/badge/counter/signature specs in zones only, never unconditional DNA fields), FR-340 (empty-zone rule: zones with no quoted text LEFT OUT, never invented graphics), FR-313 amendment (counter detection now IMPLEMENTED in `CounterSpec` dict). Paid run `20260820_145809_4a0q` exposed three defects: invented page-number chip from unconditional STYLE_DNA, empty-zone graphics, duplicated row text — fixed by these four FRs + amendment. Glossary gains Counter rule, Gated-zone rule, Empty-zone rule entries; Meta-style note on gated-zone specs. Future sessions (D60–D63) block FR-341–355. Prior: v2.4.1/D58 compress pin withdrawn, v2.4.0/D56–D57 SESSION I registry 9→19 + LLM-matched assignment + teal spine, v2.3.0/D54–D55 SESSION H compress mode + style doubling-down.
