...
**Last updated:** 2026-08-21 (v2.7.0/D63, SESSION N: output language — `run.copy_language_mode` source|target (brand configs `target`), `--copy-language`, the three-rung language ladder (Virlo `language_detected` → slide-intel deck `language` → unknown), `copywrite._call_translate` + `prompts/copy_translate_system.md` (eleventh global template, `SHIPPED_COUNT` 16) translating never shortening, translate-before-auto, `copy_language`/`source_language`/per-row `translated` receipts + tags `copy_not_translated`/`translate_length_drift`, `plan.off_language_post` bind screen + mode-gated LANG skip (FR-343–346); FR-313 bare-numeral rule; `big-number-editorial` match_profile narrowed (FR-341); `plans/tools/splice_builtin_twin.py`. Prior: v2.6.0/D62, SESSION M: cover best-of-3 — `run.cover_candidates` 1–3 (brand configs 3), `hypesocials/cover_pick.py` + `prompts/cover_pick_system.md` (tenth global role template), carousel submit/commit split with a per-candidate FR-317 ledger, `covers/` store, `meta.yaml.cover_pick` + gallery strip, estimator lines (FR-351/352/354); `carousel_copy_mode: auto` with the pure `_rows_over_budget` (FR-353; brand configs pin `auto`, engine default `verbatim`); launch-summary `carousels` line (FR-333 display obligation finally met); registry: `big-number-editorial` and `contrast-verdict-deck` single-ground, numeral rule re-worded (SESSION L defects 1–2). Glossary gains Cover best-of-3, Auto copy mode, Parity grounds. Deferred to N: FR-313's detector on bare `01…07` panel lines. Prior: v2.5.2/D61 SESSION L (seven carousel-derived styles, FR-355), v2.5.1/D60 SESSION K (palette/type contracts, house spine, 2K), v2.5.0/D59 SESSION J (FR-338/339/340), v2.4.x D56–D58.
