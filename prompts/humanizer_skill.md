<!--
VENDORED REFERENCE — DO NOT WIRE THIS FILE INTO THE ENGINE.

Source:  https://github.com/blader/humanizer  (SKILL.md at the repo root, branch `main`,
         fetched 2026-08-20 from
         https://raw.githubusercontent.com/blader/humanizer/main/SKILL.md)
License: MIT, Copyright (c) 2025 Siqi Chen. The text below is reproduced VERBATIM;
         only this comment header was added.
Basis:   Wikipedia's "Signs of AI writing" (WikiProject AI Cleanup), 35 patterns.

Reference-only: the engine never loads this file. It is not a template, it has no
`_ALLOWLIST` row and no `_BUILT_INS` twin, and no prompt interpolates it — an external
instruction file reaching a model through the prompt path is exactly the drift the
per-role allowlist exists to stop (D54 / FR-332).

The distilled ON-IMAGE subset — the ~14 patterns that can actually go wrong in a
compressed carousel line or caption — lives in `prompts/copy_compress_system.md` and its
`prompts_engine._BUILT_INS` twin. That template is the single source of truth for what the
copy model is told. This file is what its editor reads: when the upstream patterns change,
re-vendor here first, then re-derive the subset there in the same commit.
-->
